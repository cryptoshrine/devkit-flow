...
create_backup() {
    ...
    # Create exclude arguments for cp
    local EXCLUDE_ARGS=""
    for item in "${EXCLUDE[@]}"; do
        EXCLUDE_ARGS="$EXCLUDE_ARGS --exclude=$item"
    done
    
    # Copy files and directories individually, excluding specified directories
    rsync -av --progress $EXCLUDE_ARGS "${PROJECT_ROOT}/" "${BACKUP_DIR}/" || {
        warning "Some files could not be copied, but continuing with backup"
    }
